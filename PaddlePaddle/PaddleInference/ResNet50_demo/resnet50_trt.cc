...
int main(int argc, char** argv) {
  ...
  // 启用 GPU
  config.EnableUseGpu(100, 1);
  
  config.EnableTensorRtEngine(1 << 30, 1, 3,
                                PrecisionType::kFloat32, false, false);

  config.CollectShapeRangeInfo("./shape.txt");

  config.EnableTunedTensorRtDynamicShape("./shape.txt");
  ...
  auto input_tensor_0 = predictor->GetInputHandle(input_names[0]);

  // 设置输入 Tensor 的维度信息
  std::vector<int> input_shape_0 = {1,3,224,224};
  input_tensor_0->Reshape(input_shape_0);

  // 准备输入数据
  std::vector<float> input_data_0(1*3*224*224, 1.0);

  // 设置输入 Tensor 数据
  input_tensor_0->CopyFromCpu(input_data_0.data());
