...
实际上，输出的 format_tag 总是 nc （For the destination tensor the memory format is always dnnl::memory::format_tag::nc）
*******************************************************************************/
...
void inner_product_example(dnnl::engine::kind engine_kind) {
    ...
    auto user_src_md = memory::desc(src_dims, dt::f32, tag::nchw);
    auto user_weights_md = memory::desc(weights_dims, dt::f32, tag::oihw);
    auto user_bias_md = memory::desc(bias_dims, dt::f32, tag::a);
    auto user_dst_md = memory::desc(dst_dims, dt::f32, tag::nc);
    ...
    auto inner_product_src_md = memory::desc(src_dims, dt::f32, tag::any);
    auto inner_product_weights_md = memory::desc(weights_dims, dt::f32, tag::any);
    ...
    auto inner_product_pd = inner_product_forward::primitive_desc(engine,
            ...
            inner_product_bias_md, inner_product_dst_md);
