...
int main(int argc, char** argv) {
  ...
  std::vector<std::vector<std::string>> batch_input_tensor_values(input_names.size());
  ...
  for(int i = 0; i < input_names.size(); i++) {
    ...
    batch_input_tensor_values[i].resize(num_elements_per_batch, "hello");
    // std::generate(batch_input_tensor_values[i].begin(), batch_input_tensor_values[i].end(), "hello");  // generate random numbers in the range [0, 255]
    batch_input_tensors.push_back(Ort::Experimental::Value::CreateTensor<std::string>(batch_input_tensor_values[i].data(), batch_input_tensor_values[i].size(), input_shape));

    // double-check the dimensions of the input tensor
    assert(batch_input_tensors[i].IsTensor() &&
           batch_input_tensors[i].GetTensorTypeAndShapeInfo().GetShape() == input_shape);
    cout << "batch_input_tensor shape: " << print_shape(batch_input_tensors[i].GetTensorTypeAndShapeInfo().GetShape()) << endl;
  }
  ...
  try {
    ...
    for(int i = 0; i < input_names.size(); i++) {
      assert(batch_output_tensors.size() == output_names.size() &&
              batch_output_tensors[i].IsTensor() &&
              batch_output_tensors[i].GetTensorTypeAndShapeInfo().GetShape()[i] == batch_size);
