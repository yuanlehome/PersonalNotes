...
int main(int argc, char** argv) {
  ...
  // onnxruntime setup
  Ort::Env env(ORT_LOGGING_LEVEL_ERROR, "example-model-explorer");
  ...
  try {
    ...
    auto start_time = util::time();
    for(int i = 0; i < repeat_times; i++)
      output_tensors = session.Run(session.GetInputNames(), input_tensors, session.GetOutputNames());
    cout << "time cost: "  << util::time_diff(start_time, util::time()) / repeat_times << "ms" << endl;
    cout << "done" << endl;
